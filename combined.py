...
def input_picture():
    ...
    robot_pos = (robot_position[0]-5, robot_position[1]-5, 10, 10, 0)          # (x1, y1, w, h, direction)
...
def detect_aruco_markers(frame):
    ...
    return marker_corners, marker_centers, robot_corners
...
class CameraCropper:
    def __init__(self, camera_index=0, show_feed=True):
    ...
    def initialize(self):
        """Initialize the camera and calibration."""
        ...
        print("Press 'q' to quit, 'c' to recalibrate, 's' to save the current cropped image")
        return True
    ...
    def get_cropped_image(self, save_image=False, filename=None):
        ...
        if not self.is_initialized:
            raise RuntimeError("CameraCropper not initialized. Call initialize() first.")
            
        ret, frame = self.cap.read()
        if not ret:
            raise RuntimeError("Could not read frame from camera.")
            
        # Undistort the image if calibration is available
        if self.camera_matrix is not None and self.dist_coeffs is not None:
            frame = undistort_image(frame, self.camera_matrix, self.dist_coeffs)
            
        # Detect markers
        marker_corners, marker_centers, robot_corners = detect_aruco_markers(frame)
        
        # Get robot position
        robot_position = marker_centers.get(0, None)
        ...
        cropped_image = None
        cropped_display = None
        if crop_coords is not None:
            x1, y1, x2, y2 = crop_coords
            cropped_image = frame[y1:y2, x1:x2]
            cropped_display = cropped_image.copy()
            
            # Update robot position relative to cropped image if robot is detected
            if robot_position is not None:
                robot_x, robot_y = robot_position
                robot_position_in_crop = (robot_x - x1, robot_y - y1)
                
                # Draw robot position on cropped display
                if 0 <= robot_position_in_crop[0] < cropped_display.shape[1] and 0 <= robot_position_in_crop[1] < cropped_display.shape[0]:
                    cv2.circle(cropped_display, robot_position_in_crop, 5, (0, 0, 255), -1)
                    cv2.putText(cropped_display, "Robot", (robot_position_in_crop[0] + 10, robot_position_in_crop[1] - 10), 
                              cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                
            # Update robot corners relative to cropped image if robot is detected
            if robot_corners is not None:
                robot_corners = [(x - x1, y - y1) for x, y in robot_corners]
            
            # Save the cropped image if requested
            if save_image and cropped_image is not None:
                if filename is None:
        ...
        if key == ord('q'):
            ...
            if cropped_image is not None:
                ...
                print("No cropped image available to save.")
                
        return cropped_image, robot_position_in_crop, robot_corners
    
    def run_live_feed(self):
        """Run a continuous live feed, showing detections and processing keyboard commands."""
        if not self.is_initialized:
            self.initialize()
            
        print("Live feed running. Press 'q' to quit, 'c' to recalibrate, 's' to save the current cropped image")
        
        try:
            while True:
                cropped_image, robot_position, robot_corners = self.get_cropped_image()
                
                # Display robot position if detected
                if robot_position is not None:
...
if __name__ == '__main__':
    cropper = CameraCropper(camera_index=0, show_feed=True)
    cropper.initialize()
    input_picture()